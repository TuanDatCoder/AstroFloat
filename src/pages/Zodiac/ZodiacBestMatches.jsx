...
const CATEGORY_MAP = [
  { id: 'love', name: 'Hợp Để Yêu', desc: 'Có sự đồng điệu về mặt tình cảm và cảm xúc', dbCats: ['Tình yêu'], icon: Heart, border: 'border-pink-500/30', bg: 'bg-pink-950/40', text: 'text-pink-400', gradient: 'from-pink-400 to-purple-400' },
  { id: 'friend', name: 'Hợp Làm Bạn', desc: 'Sở hữu chung năng lượng xã hội và giao tiếp', dbCats: ['Xã hội'], icon: Users, border: 'border-blue-500/30', bg: 'bg-blue-950/40', text: 'text-blue-400', gradient: 'from-blue-400 to-cyan-400' },
  { id: 'work', name: 'Hợp Trong Công Việc', desc: 'Gắn kết và thăng hoa trong sự nghiệp, tài chính', dbCats: ['Sự nghiệp', 'Tiền bạc', 'Sự nghiệp & Tiền bạc'], icon: Briefcase, border: 'border-emerald-500/30', bg: 'bg-emerald-950/40', text: 'text-emerald-400', gradient: 'from-emerald-400 to-teal-400' },
  { id: 'personality', name: 'Đồng Điệu Tính Cách', desc: 'Giống nhau ở phần góc khuất và cái tôi sâu thẳm', dbCats: ['Tính cách', 'Góc tối', 'Góc khuất'], icon: Zap, border: 'border-purple-500/30', bg: 'bg-purple-950/40', text: 'text-purple-400', gradient: 'from-purple-400 to-indigo-400' },
];
  ...
  return (
    <div className="relative inline-block w-full min-w-[300px] text-left">
      <div 
        ...
        className={`w-full bg-slate-900/60 backdrop-blur-xl border border-indigo-500/30 rounded-2xl px-6 py-4 cursor-pointer flex justify-between items-center transition-all ${isOpen ? 'shadow-[0_0_20px_rgba(99,102,241,0.3)] border-indigo-400' : 'hover:border-indigo-500/50 hover:shadow-[0_0_15px_rgba(99,102,241,0.2)]'}`}
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className={`absolute z-50 w-full mt-3 bg-slate-900/95 backdrop-blur-2xl border border-indigo-500/30 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] overflow-hidden max-h-64 overflow-y-auto`}
...
export default function ZodiacBestMatches() {
  ...
  return (
    <div className="flex flex-col items-center pt-24 pb-20 px-6 relative z-10 w-full max-w-6xl mx-auto min-h-screen">
      <Link to="/zodiac" className="absolute top-24 left-6 flex items-center gap-2 text-cyan-400 hover:text-cyan-300 transition-colors uppercase tracking-widest text-[10px] font-black bg-cyan-950/50 px-4 py-2 rounded-full border border-cyan-500/20">
        <ArrowLeft className="w-3 h-3" /> Quay Lại
      </Link>

      <Link to={`/zodiac-all-matches?sign=${selectedSignId}`} className="absolute top-24 right-6 flex items-center gap-2 text-amber-400 hover:text-amber-300 transition-colors uppercase tracking-widest text-[10px] font-black bg-amber-950/50 px-4 py-2 rounded-full border border-amber-500/20">
        Toàn Bộ BXH <ArrowLeft className="w-3 h-3 rotate-180" />
      </Link>
      ...
      {loading ? (
        ...
        <div className="text-center bg-slate-900/40 p-8 rounded-[2rem] border border-white/5">
          ...
          {CATEGORY_MAP.map((cat) => {
            ...
            return (
              <motion.div 
                ...
                className={`relative overflow-hidden p-8 rounded-[2.5rem] border ${cat.border} ${cat.bg} backdrop-blur-xl shadow-2xl flex flex-col`}
              >
                {/* Background Decor */}
