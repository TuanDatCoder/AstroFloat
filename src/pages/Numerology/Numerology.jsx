  ...
  return (
    <div className="flex flex-col items-center pt-10 pb-20 px-6 relative z-10 w-full max-w-7xl mx-auto min-h-[80vh]">
      ...
      <motion.div
        ...
        className="mb-12 w-full max-w-lg mx-auto bg-slate-900/60 backdrop-blur-md p-6 md:p-8 rounded-[2rem] border border-purple-500/20 shadow-2xl flex flex-col sm:flex-row gap-4 items-center relative z-20"
      >
      ...
      <AnimatePresence mode="wait">
        {error && (
          <motion.p 
            ...
            className="text-red-400 mb-8 bg-red-950/40 px-6 py-3 rounded-xl border border-red-500/20 backdrop-blur-md"
          >
            Lỗi: {error}
          </motion.p>
        )}
        ...
        {loading ? (
          ...
          <motion.div 
            ...
            className="flex flex-col items-center p-12 bg-white/[0.03] backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl w-full max-w-xl"
          >
            ...
            {data.map((item, index) => (
              <Link to={`/numerology/${item.number}`} key={item.number || index} className="block">
                <motion.div
                  ...
                  className="transform-gpu will-change-transform group p-[1px] rounded-[2rem] overflow-hidden bg-gradient-to-b from-white/10 to-transparent shadow-xl h-full"
                >
                  ...
                  <div className="relative h-full px-8 py-10 bg-slate-900/90 backdrop-blur-sm md:backdrop-blur-md rounded-[31px] flex flex-col justify-between border border-white/5 transition-colors duration-500 group-hover:bg-slate-900/70">
                    <motion.div
                      animate={{ y: [-4, 4, -4] }}
                      transition={{ duration: 6 + (index % 3), repeat: Infinity, ease: "easeInOut", delay: (index % 3) * 0.5 }}
                    >
