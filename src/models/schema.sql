...
CREATE TABLE zodiac_signs (
    ...
    description TEXT,
    image_url TEXT            -- Nơi chứa link ảnh 3D/Pixar cực mượt của bạn
);
...
CREATE TABLE compatibility (
    ...
    friendship_analysis TEXT  -- Phân tích về tình bạn/công việc
);
...
-- ==========================================
-- UPDATE: BỔ SUNG TRƯỜNG DỮ LIỆU CUNG HOÀNG ĐẠO
-- ==========================================
...
-- ==========================================
-- 6. BẢNG MỚI: CHI TIẾT THẦN SỐ HỌC (Numerology Details)
-- ==========================================
CREATE TABLE numerology_details (
    id SERIAL PRIMARY KEY,
    number INT REFERENCES numerologies(number) ON DELETE CASCADE,
    topic VARCHAR(100),   -- Ví dụ: 'Tình duyên', 'Sự nghiệp', 'Thách thức'
    title VARCHAR(255),   -- Ví dụ: 'Cách số 1 yêu', 'Đỉnh cao sự nghiệp'
    content TEXT,         -- Nội dung chi tiết bài viết
    icon_name VARCHAR(50) -- Tên icon để hiển thị trên React (ví dụ: 'Heart', 'Briefcase')
);

-- ==========================================
-- 7. BẢNG MỚI: CHI TIẾT CUNG HOÀNG ĐẠO (Zodiac Details)
-- ==========================================
CREATE TABLE zodiac_details (
    id SERIAL PRIMARY KEY,
    zodiac_id INT REFERENCES zodiac_signs(id) ON DELETE CASCADE,
    topic VARCHAR(100),   -- Ví dụ: 'Truyền thuyết', 'Góc tối', 'Sở thích'
    title VARCHAR(255),
    content TEXT,
    is_premium BOOLEAN DEFAULT FALSE -- Để sau này bạn làm tính năng nạp tiền
);
