...
export default function PinnacleDetailClient({ number, initialPinnacle }) {
  const [pinnacle, setPinnacle] = useState(initialPinnacle);
  const [loading, setLoading] = useState(!initialPinnacle);
  const [error, setError] = useState(null);

  useEffect(() => {
    async function fetchPinnacle() {
      if (initialPinnacle) return;
      try {
        setLoading(true);
    ...
    fetchPinnacle();
  }, [number, initialPinnacle]);

  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] w-full">
        <div className="w-16 h-16 border-4 border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin mb-4" />
        <p className="text-indigo-300 font-medium tracking-widest animate-pulse uppercase text-xs">Đang truy xuất năng lượng đỉnh cao...</p>
      </div>
    );
  }

  return (
    <div className="flex flex-col items-center pt-32 pb-32 px-6 relative z-10 w-full max-w-4xl mx-auto min-h-screen">
      
      <Link href={ROUTES.DISCOVER} className="self-start mb-12 flex items-center gap-2 text-indigo-400 hover:text-indigo-300 transition-colors text-sm font-bold uppercase tracking-widest group">
        <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> QUAY LẠI KHÁM PHÁ
      </Link>
      ...
      {error || !pinnacle ? (
        ...
        >
          ...
          <div className="bg-slate-900/70 p-10 md:p-16 rounded-[4rem] border border-white/10 shadow-2xl relative overflow-hidden group">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(99,102,241,0.1)_0%,_transparent_60%)] pointer-events-none" />
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-white mb-4 leading-tight italic uppercase">
                  {pinnacle.title}
                </h1>
              </div>
            </div>
