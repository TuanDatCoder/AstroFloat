                ...
                <div className="flex justify-between items-start mb-6">
                   <div className="w-14 h-14 bg-black/40 border border-white/10 rounded-2xl flex items-center justify-center text-2xl font-black text-indigo-400">
                    ...
                    <div className="flex gap-2 text-indigo-400">
                       <Link 
                         href={`/admin/pinnacles/details/${item.number}`} 
                         className="p-2 hover:bg-indigo-500/10 rounded-lg transition-colors"
                         title="Quản lý bài viết chuyên sâu"
                       >
                         <BookOpen className="w-4 h-4" />
                       </Link>
                       <Link href={`/admin/pinnacles/edit/${item.number}`} className="p-2 hover:bg-white/5 rounded-lg transition-colors"><Edit2 className="w-4 h-4" /></Link>
                       <button onClick={() => handleDelete(item.number)} className="p-2 hover:bg-rose-500/10 text-rose-400 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </div>
                </div>
