...
import { 
  ArrowLeft, Plus, Trash2, Edit3, X, Sparkles, 
  Filter, Hash, CheckCircle2, Save, Loader2 
} from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-32">
      ...
      {(isAdding || editingId) && (
        <motion.div 
          ...
          className="bg-slate-900 border border-cyan-500/30 p-8 rounded-[2.5rem] shadow-2xl relative z-10"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {loading ? (
            ...
            <div key={m.id} className="bg-slate-900/40 border border-white/5 p-6 rounded-[2rem] hover:border-white/10 transition-all group">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-xs font-bold text-white flex items-center gap-1">
                         <Hash className="w-3 h-3" /> {m[FIELD_NAME_ADVANCED_METRICS.NUMBER]}
                       </span>
                    </div>
               ...
               <p className="text-xs text-gray-500 line-clamp-2 italic font-light leading-relaxed mb-4">{m[FIELD_NAME_ADVANCED_METRICS.CONTENT]}</p>
               <div className="flex items-center gap-2 pt-3 border-t border-white/5">
                  <CheckCircle2 className="w-3 h-3 text-emerald-500/40" />
                  <span className="text-[8px] font-bold text-gray-600 uppercase tracking-widest">Active Metric</span>
               </div>
            </div>
          ))}
        </div>
