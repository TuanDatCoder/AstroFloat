...
export default function AdminZodiacMatchFormClient({ id }) {
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => ({ 
      ...prev, 
      [name]: type === 'number' || name === 'match_score' ? parseInt(value) || 0 : value 
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push('/admin/zodiac-matches');
    } catch (error) {
      console.error("Save error:", error);
      alert("Lỗi khi lưu: " + (error.message || "Cặp này có thể đã tồn tại."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4">
        <Link href="/admin/zodiac-matches" className="p-2 hover:bg-white/5 rounded-lg text-gray-400">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-4 pt-6">
          <button 
            type="button" onClick={() => router.push('/admin/zodiac-matches')}
